...
%   tn-and-ss    - mode that produces both tutornotes and specimen
%                  solutions in separate PDFs files.
...
\tntitle[Title of SS]{Title of TN}
\tnyear{2024J}
\copyrightyear{2024} % If different from \abyear
...
\tma[12 November 2024]{01}[Covers units A and B]
...
\subtotal*
\item You can use decimals in marks. \verb!\mk{1.5}! gets rendered as \mk{1.5}

Another fractional mark \mk{.5}

\subtotal*

\item If you really want, other decimals can also be used.\mk{.2}

Another example, so the part-marks add up to 0.5.\mk{0.3}

\subtotal*[shows halves too]
\end{enumerate}
...
\begin{enumerate}
  \item The lines after a \verb!subtotal! command can be customized using
    ...
    are \verb!moveleft!, \verb!movedown!, \verb!width!, \verb!height!, \verb!color!, 
    and \verb!draw line!. 
    ...
    \begin{itemize}
      ...
      \item \verb!\setSubtotalHline[moveleft=-2cm,width=.5\textwidth]!
        ...
        \subtotal*
      \item \verb!\setSubtotalHline[color=green,movedown=20pt]!
        \setSubtotalHline[color=green,movedown=20pt] \mk{2}
  ...
  \item The lines after a \verb!total! command can be customised 
    ...
    \begin{itemize}
      ...
      \item \verb!\setTotalHline[moveleft=2cm,width=.75\textwidth]!
        ...
        \total*
      \item \verb!\setTotalHline[color=purple,movedown=1cm]!
        \setTotalHline[color=purple,movedown=1cm] 
...
And then you can overwrite certain aspects as you see fit, for example \verb!\ourule[color=blue,movedown=-1cm]!

\ourule[color=blue,movedown=0.5cm]
...
\begin{enumerate}
  \item The \verb|\tnonly| command \emph{only} produces output in the tutor version of the tutor notes, 
    ...
    \tnonly{ {\color{blue} students will not see this!}}.
  \item The \verb|tutor| environment is only visible in tutor notes, 
    ...
    \end{tutor}
    \tnpagebreak
  \item The \verb|\ssonly| command \emph{only} produces output when \verb|specsolns| is active; for example \ssonly{this is only output in student solutions}
  ...
  \item The \verb|\tnorss| takes two parameters, the first will only be seen in the tutor version (when \verb|specsolns|
    ...
    For example: \tnorss{for tutors!}{for students!}
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\assignment{QUIZ}[1 April 2025]{01}[Custom assessment type]
\samepageassignment{QUIZB}[1 April 2025]{01}[Custom assessment type]
