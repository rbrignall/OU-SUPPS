...
\end{desc}
Sets the title for the |tutor| environment (optional).
The |tutor| environment \emph{only} outputs when |specsolns| is \emph{not} active; the title is, by default,
|{\bfseries For tutors only:}\par| but this can be changed using |\tutoronlytitle|. See page~\pageref{outn:tutoronly}
for further detail.\label{outn:tutoronlytitle}

\begin{desc}
|\studentonlytitle{<title for student only environment>}|
\end{desc}
Sets the title for the |student| environment (optional).
The |student| environment \emph{only} outputs when |specsolns| \emph{is} active; the title is, by default,
|{\bfseries For students only:}\par| but this can be changed using |\studentonlytitle|. See page~\pageref{outn:studentonly}
for further detail.\label{outn:studentonlytitle}
...
output to tutor notes when the |specsolns| is \emph{not} active.
...
output to tutor notes when the |specsolns| is \emph{not} active.\label{outn:tutoronly}
See also |\tutoronlytitle| on page~\pageref{outn:tutoronlytitle} for the title of this environment.

\begin{desc}
|\studentonly{<commands/text>}|
\end{desc}
Commands, text, and anything else passed to the |\studentonly| command will only
output to student notes when the |specsolns| \emph{is} active.\label{outn:studentonly}
...
output to student notes when the |specsolns| \emph{is} active.
See also |\studentonlytitle| on page~\pageref{outn:studentonlytitle} for the title of this environment.
...
This option is available for drafting exams, on machines where the package containing |barcodes.sty| does not work properly. In most cases, the DPU can insert the requisite barcode later.
...
|\mkplus{<n>}{<description>}|
\end{desc}
Some equivalent commands that produce the same effect as |\mk[<description>]{<n>}|. These exist primarily for backward-compatibility with OUTeX.
